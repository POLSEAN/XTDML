...
#'
#' @description
#' Double machine learning (DML) for partially linear regression models with fixed effects.
#'
#' @format [R6::R6Class] object inheriting from [dml_hybrid].
#'
#' @family dml_hybrid
#' @details
...
#'
#' @export
dml_hybrid_plr = R6Class("dml_hybrid_plr",
   inherit = dml_hybrid, public = list(
    #' @description
    #' creates a new instance of this R6 class.
    #'
    #' @param data (`dml_hybrid_data`) \cr
    #' The `dml_hybrid_data` object providing the data and specifying the
    ...
    initialize = function(data,
      ...
      super$initialize_double_ml(
          ...
          private$learner_ = list("ml_l" = ml_l,"ml_m" = ml_m)
    ...
    #' @description
    #' Set hyperparameters for the nuisance models of DML models with fixed effects.
    #'
    ...
    #' @description
    #' Hyperparameter-tuning for DML models with fixed effects.
    #'
  ...
  private = list(
    ...
    nuisance_est = function(smpls, ...) {
        ...
        m_hat_initial = dml_cv_predict(self$learner$ml_m,
        ...
        l_hat_initial = dml_cv_predict(self$learner$ml_l,
        ...
        d_raw = self$data$data_model[[self$data$treat_col]]
        y_raw = self$data$data_model[[self$data$y_col]]
        idx = self$data$data_model[[self$data$cluster_cols]]

        if(self$model == "wg"){
            m_hat = wg_transformation(m_hat_initial,idx)
            l_hat = wg_transformation(l_hat_initial,idx)
            d = wg_transformation(d_raw,idx)
            y = wg_transformation(y_raw,idx)
        }else if(self$model == "fd"){
            m_hat = fd_transformation(m_hat_initial,idx)
            l_hat = fd_transformation(l_hat_initial,idx)
