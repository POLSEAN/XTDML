#' @title Abstract class dml_hybrid
#'
#' @description
...
dml_hybrid = R6Class("dml_hybrid",
  active = list(
    ...
    #' A `character(1)`  specifying the data trasformation `"wg"`, `"fd"`.
    #' Default is "`wg`".
  ...
  public = list(
    ...
    initialize = function() {
      stop("dml_hybrid is an abstract class that can't be initialized.")
    },
    ...
    print = function() {
      ...
      data_info = paste0(
        ...
        "No. Clusters: ", private$n_clusters)  #N(T-1) for FD private$
    ...
    fit = function(store_predictions = FALSE) {
      ...
      for (i_rep in 1:self$n_rep) {
        ...
        for (i_treat in 1:self$data$n_treat) {
          ...
          # compute standard errors for causal parameters (theta_hat)
      ...
      if (self$apply_cross_fitting) {
        ...
        for (i_rep in 1:self$n_rep) {
          ...
          for (i_var in 1:self$data$n_cluster_vars) {
            ...
            private$n_clusters = length(clusters)   ###added private$

            dummy_task = Task$new(
              "dummy_resampling", "regr",
               data.table(dummy_var = rep(0, private$n_clusters)))   #added private$
    ...
    #' @description
    #' Summary for DML models with CRE after calling `fit()`.
    #'
    ...
    #' @description
    #' Get hyper-parameters for the nuisance model.
    #'
    #' @param learner (`character(1)`) \cr
    #' The nuisance model/learner (see method `params_names()`)
    #'
  ...
  private = list(
    ...
    n_clusters = NA_integer_,               ##added 02oct23
    smpls_cluster_ = NULL,
    ...
    initialize_double_ml = function(data,
      ...
      assert_class(data, "dml_hybrid_data")
    ...
    est_causal_pars_theta = function() {
      ...
      test_cluster_inds = smpls_cluster$test_ids
      ...
      if (self$data$n_cluster_vars == 1) {
        ...
        for (i_fold in 1:self$n_folds) {   ##n_folds
          ...
          for (cluster_value in I_k) {
            ...
            gamma_hat = gamma_hat + const * sum_na(outer(
                        psi_theta[ind_cluster],
                        psi_theta[ind_cluster]))
          }
          j_hat = j_hat + sum_na(psi_theta_a[test_inds]) / length(I_k)
        }
      ...
      if (dml_procedure == "dml1") {
        ...
        for (i_fold in seq_len(length(test_ids))) {
          ...
          psi_theta_a_subsample_mean = psi_theta_a_subsample_mean +
            scaling_factor * sum(psi_theta_a[test_index], na.rm = TRUE)
          psi_theta_b_subsample_mean = psi_theta_b_subsample_mean +
            scaling_factor * sum(psi_theta_b[test_index], na.rm = TRUE)
        }
