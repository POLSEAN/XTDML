...
#' `dml_hybrid_data` construct a synthetic data set suitable for panel data analysis.
#'
#' `dml_hybrid_data` objects can be initialized from a
#' [data.table][data.table::data.table()]. Alternatively `dml_hybrid` provides
...
dml_hybrid_data = R6Class("dml_hybrid_data",
                       active = list(
                         ...
                         #' @field dbar_cols (`character()`)\cr
                         #' The treatment variable(s).
                         dbar_cols = function(value) {
                           if (missing(value)) {
                             return(private$dbar_cols_)
                           } else {
                             dbar_cols = value # to get more meaningful assert error messages
                             reset_value = !is.null(self$data_model)
                             assert_character(dbar_cols, unique = TRUE)
                             assert_subset(dbar_cols, self$all_variables)
                             private$dbar_cols_ = dbar_cols
                             if (reset_value) {
                               private$check_disjoint_sets()
                               self$set_data_model(self$dbar_cols[1])
                             }
                           }
                         },
                         ...
                         #' the user via `y_col`, `d_cols`, `x_cols`, `dbar_cols`, `xbar_cols` and `z_cols`.
                         ...
                         #' @field n_treat_bar (`integer(1)`) \cr
                         #' The number of treatment variables.
                         n_treat_bar = function(value) {
                           if (missing(value)) {
                             return(length(self$dbar_cols))
                           } else {
                             stop("can't set field n_treat_bar")
                           }
                         },
                         ...
                         #' @field treat_bar_col (`character(1)`) \cr
                         #' "Active" treatment variable in the multiple-treatment case.
                         treat_bar_col = function(value) {
                           if (missing(value)) {
                             return(private$treat_bar_col_)
                           } else {
                             stop("can't set field treat_bar_col")
                           }
                         },
                         ...
                         #' @field xbar_cols (`character()`) \cr
                         #' The average of the covariates.
                         xbar_cols = function(value) {
                           if (missing(value)) {
                             return(private$xbar_cols_)
                           } else {
                             xbar_cols = value
                             reset_value = !is.null(self$data_model)
                             if (!is.null(xbar_cols)) {
                               assert_character(xbar_cols, unique = TRUE)
                             }

                             if (!is.null(xbar_cols)) {
                               assert_subset(xbar_cols, self$all_variables)
                               private$xbar_cols_ = xbar_cols
                             }
                             if (reset_value) {
                               private$check_disjoint_sets()
                               self$set_data_model(self$dbar_cols[1])
                       ...
                       public = list(
                         ...
                         #'
                         #' @param dbar_cols (`NULL`, `character()`) \cr
                         #' The mean of the treatment.
                         #'
                                               ...
                                               cluster_cols = NULL)
                         {
                           ...
                           self$d_cols = d_cols
                           self$x_cols = x_cols
                           self$xbar_cols = xbar_cols
                           self$dbar_cols = dbar_cols
                           private$check_disjoint_sets()
                         ...
                         #' @description
                         #' Print `dml_hybrid_data` objects.
                         print = function() {
                           header = "================= DML-CRE Object ==================\n"
                           data_info = paste0(
                             ...
                             "Mean covariates: ", paste0(c(self$xbar_cols, self$dbar_cols), collapse = ", "), "\n",
                         ...
                         set_data_model = function(treatment_var) {
                           ...
                           private$treat_bar_col_ = self$dbar_cols ###
                       ...
                       private = list(
                         ...
                         check_disjoint_sets = function() {
                           ...
                           x_cols    = self$x_cols
                           d_cols    = self$d_cols
                           xbar_cols = self$xbar_cols
                           dbar_cols = self$dbar_cols
...
#' The covariates.
#'
#' @param dbar_cols (`character()`) \cr
#' The mean of the treatment variable(s).
#'
#' @param xbar_cols (`character()`) \cr
#' The mean of the covariates.
...
#'
#' @return Creates a new instance of class `dml_hybrid_data`.
#'
#' @export
dml_hybrid_data_from_data_frame = function(df,
                           x_cols = NULL, y_col = NULL, d_cols = NULL,
                           xbar_cols = NULL,  dbar_cols = NULL,
                           cluster_cols = NULL)
{
  if (is.null(cluster_cols)) {
    stop(print("Specify at least one (`cluster_vars`)."))
  } else {
    data = dml_hybrid_data$new(df,
                            x_cols = x_cols, y_col = y_col, d_cols = d_cols,
                            xbar_cols = xbar_cols, dbar_cols = dbar_cols,
